...
(function ($) {
	...
	var Drum = function (element, options, transformProp) {
		...
		var transform = function () {
			...
			//IE 10-11 fix for preserve-3d;
			$(drum).css("-ms-transform", "none");
			//console.log('translateZ(-' + settings.radius + 'px) ' + settings.rotateFn + '(' + settings.rotation + 'deg)');
			...
			if (selected) {
				...
				if (isIE()) {
					for (var i in settings.mapping) {
						var angle = -settings.mapping[i].angle + settings.rotation;
						$(settings.mapping[i].elem).css(
							settings.transformProp,
							settings.rotateFn + '(' + angle + 'deg) translateZ(' + settings.radius + 'px)'
						);
					}
				}
